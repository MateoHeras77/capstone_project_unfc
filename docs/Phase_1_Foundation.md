...
## Purpose
To establish a persistent and efficient data layer that bridges external market data (via `yfinance`) with an internal database (`Supabase`). This phase focuses on the "Smart Retrieval" logic at a **Weekly** scale.
...
2.  **Smart Fetcher Utility:** Develop a Python script/service using `yfinance` that:
    ...
    *   Fetches missing data using `interval='1wk'`.
...
*   **Weekly Interval:** The system uses weekly data pulls (`1wk`).
*   **Stale Data Check:** Logic to identify if the last record in the database aligns with the most recent full week.
...
*   **Duplicate Prevention:** Run the fetcher twice for the same asset and verify no duplicate primary keys (asset_id + timestamp) are created.
