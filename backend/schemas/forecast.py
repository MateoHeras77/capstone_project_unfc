...
class ForecastMetricsRequest(BaseModel):
    ...
    confidence_level: float = Field(default=0.95, ge=0.5, le=0.99)
    models: Optional[List[Literal["base", "prophet", "lstm", "chronos2"]]] = Field(
        default=None,
        description="Models to run. Default is base+prophet only for faster response; use all four for full comparison.",
    )
    bounds_horizon_periods: Optional[int] = Field(
        default=None,
        ge=1,
        le=52,
        description="Forecast bounds horizon (number of periods). If not set, uses 12 for 1wk and 4 for 1mo. Should match the chart's selected horizon.",
    )
