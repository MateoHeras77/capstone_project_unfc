...
def _run_walk_forward_one_step(
    prices: pd.Series,
    model_name: Literal["base", "prophet", "lstm", "chronos2", "prophet_xgb"],
    last_n: int,
    ...
    for k in range(last_n):
        ...
        try:
            if model_name == "base":
                ...
                cfg = Chronos2Config()
                cfg.context_length = min(256, len(train))
                model = Chronos2Forecaster(confidence_level=confidence_level, config=cfg)
...
def walk_forward_backtest_last_n_weeks(
    ...
    epochs: int = 30,
    confidence_level: float = 0.95,
    models: Optional[List[Literal["base", "prophet", "lstm", "chronos2", "prophet_xgb"]]] = None,
    ...
    returns in ~30–60 seconds. Pass ["base", "prophet", "lstm", "chronos2"]
