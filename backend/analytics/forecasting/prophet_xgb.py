...
Forecast: step 1 = Prophet forecast + XGB residual correction; steps 2+ = Prophet only.
"""
...
class ProphetXGBForecaster(BaseForecastor):
    """
    Prophet point forecast with XGBoost residual correction for step 1.

    Loads artifact (XGBoost + scalers) from the notebook export. Prophet is
    fit on the given prices at fit() time. At forecast(), the first step
    uses Prophet + XGB residual; remaining steps use Prophet only.
    ...
    def forecast(self, periods: int = 4) -> Dict[str, Any]:
        """
        First step: Prophet forecast + XGB residual correction.
        Steps 2+ : Prophet forecast only.
        """
        ...
        # Apply XGB residual correction to step 1 only
        try:
        ...
        # Build one row: residual_lag_1..L, price_lag_1..L, vix_lag_1 (0)
        row = {}
        for lag in range(1, residual_lags + 1):
            idx = n - lag
            row[f"residual_lag_{lag}"] = float(residuals[idx]) if idx >= 0 else 0.0
        for lag in range(1, price_lags + 1):
            idx = n - lag
            row[f"price_lag_{lag}"] = float(self._prices.iloc[idx]) if idx >= 0 else 0.0
        row["vix_lag_1"] = 0.0  # backend has no VIX
        ...
        point_forecast[0] = round(point_forecast[0] + predicted_residual, 4)
